...
3. ON: **write_file** – Write to any file in the public brain repo. Provide "file_path" and "content". Paths an installed extension has given you in your private repository are written there instead. (You CANNOT write to `operations/`, `working_memory/`, `record/emails/`, or any file named `constitution.md`.)
...
8. ON: **search_email** – Search your entire email archive (headers + body). Provide "query". Optionally "with" (a name or address — letters to or from them) and "month" ("2026-07"); either works on its own with no "query", which lists the letters rather than searching them.
